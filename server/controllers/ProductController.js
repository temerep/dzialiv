...
class ProductController {
  async create(req, res) {
    try {
      const { img, name, desc, subcategory, category, pricelist } = req.body;

      if (!img || !name || !desc || !category) {
        return res.json({ message: "Required fields are not filled"});
      }
      
      const result = await Product.create({ img, name, desc, subcategory, category, pricelist })
      res.json(result);
