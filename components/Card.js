...
const Card = ({ title, badge, desc, text, price, categories, img }) => {
  return (
    <>
      {/* <Link href="#" className="flex flex-col items-center bg-white border-2 border-stone-300 rounded-xl duration-200 drop-shadow-lg md:hover:drop-shadow-none md:max-w-lg">
        <div className="flex flex-col items-start justify-start p-3">
          ...
          <h1 className="text-2xl font-bold tracking-tight text-gray-900 break-all">{title}</h1>
          </div>
          <ul className="grid grid-cols-1 lg:grid-cols-2 gap-x-4 text-emerald-600">
            {categories?.map((item) => {
              return (
                <>
                <li key={item} className="flex items-center hover:text-emerald-400">
