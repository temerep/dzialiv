      ...
      <div className="fixed z-50 p-5 overflow-x-hidden overflow-y-auto md:inset-0 max-w-full" style={{ display: selectedImage ? "block" : "none" }}>
        <div className="w-full max-w-lg">
          <div className="relative bg-white rounded-lg shadow">
            ...
            <div className="p-15 rounded-xl">{selectedImage && <Image width={1000} height={1000} src={selectedImage} alt="Збільшена фотографія" className="max-w-full max-h-screen" />}</div>
          </div>
        </div>
      </div>
      <section className="flex w-full items-center bg-gray-100 py-16">
        <div className="p-4 mx-auto max-w-7xl">
          <h2 className="pb-16 font-bold text-center  text-5xl">Фотографії</h2>
                  ...
                  {el.map((item) => {
                    return (
                      <div key={item.src} onClick={() => openModal(item.src)}>
                        <Image className="h-auto max-w-full rounded-lg" src={item.src} alt="" width={1000} height={1000}/>
                      </div>
                    );
                  })}
                </div>
